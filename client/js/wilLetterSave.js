...
(function () {
  ...
  function insertStampImage() {
    ...
    stampImage.alt = "Company stamp";
    stampImage.style.cssText =
      "float: right; width: 200px; height: 250px; margin-top: 50px; margin-right: 20px;";
  ...
  async function loadContractData() {
    ...
    document.getElementById("declarationId").value = contractData.id_number;

    document.getElementById("commencementDate").value =
      contractData.commencement_date;

    document.getElementById("endDate").value = contractData.termination_date;
  ...
  async function loadExposureAreas() {
    ...
    try {
      const response = await fetch(`${API_URL}/exposures/career/${careerId}`);
  ...
  async function saveOrUpdateParticipant(participantData) {
    ...
    if (
      ...
      if (!lookupResponse.ok) {
        throw new Error(
          lookupResult.message || "Failed to find existing participant",
        );
      }
      ...
      if (!updateResponse.ok) {
        throw new Error(
          updateResult.message || "Failed to update existing participant",
        );
      }
  ...
  async function saveContract() {
    try {
      ...
      const contractResponse = await fetch(
        `${API_URL}/contracts/participant/${participant_id}/generate`,
        {
          method: "POST",
          headers: {
      ...
      if (!contractResponse.ok) {
        throw new Error(
          contractResult.message || "Failed to generate contract",
        );
      }
      ...
      if (saveWilBtn) {
        ...
        saveWilBtn.title =
          "Click to go to Contents page to generate WIL letter";
  ...
  function addExposureToList() {
    ...
    const exists = pendingExposureAreas.some(
      (area) => area.exposure_name.toLowerCase() === name.toLowerCase(),
    );
